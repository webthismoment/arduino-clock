...
//전역함수//
int sw11(){ return digitalRead(15); } //스위치1 누르는 순간 LOW, 아니면 HIGH 반환
...
void setup() {
  ...
  pinMode(18, INPUT_PULLUP);  //커서 이동
  
  //Program setup
  setTime(13,48,20,1,6,2021);  //NOTE: 테스트 전에 현재 시각으로 설정
...
void loop() {
  //t = now();
  int sw_mode = digitalRead(14); //스위치0 누를 동안 LOW, 아니면 HIGH 반환
  int sw1 = digitalRead(15); //스위치1 누를 동안 LOW, 아니면 HIGH 반환
  int sw2 = digitalRead(16); //스위치2 누를 동안 LOW, 아니면 HIGH 반환
  int sw_cursor = digitalRead(18); //스위치3 누를 동안 HIGH, 아니면 LOW 반환
